...
for x, y, np2, pkt in zip(x_indices, results, next_pow2, max_pkt_len):
    ...
    if adjusted < pkt:
        ...
        plt.scatter(x, y, color='red', marker='x', s=80, zorder=2)

    plt.text(x, y - (0.05 * y), f"{y:.1f}", fontsize=8, ha='center', va='top', color='purple')
    plt.text(x, np2 + (0.05 * np2), f"{np2}", fontsize=8, ha='center', va='bottom', color='orange')

plt.xticks(x_indices, x_labels, rotation=45)
plt.title('Buffer Size for Different Transfer Sizes (Up to 131072 bytes)')
